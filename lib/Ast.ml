...
and Requires_sub_terms : sig
  ...
  let check_typ {ctx; ast= typ} =
    ...
    | Clf {pcf_desc; _} ->
        assert (
          ...
          | Pcf_val
              (_, _, Cfk_concrete (_, {pexp_desc= Pexp_constraint (_, Some t, _); _}))
            ->
              typ == t
  ...
  let prec_ctx ctx =
    ...
    let is_tuple_lvl1_in_constructor ty = function
      | {pcd_args= Pcstr_tuple t1N; _} -> List.exists t1N ~f:(fun carg -> carg.pca_type |> phys_equal ty)
      | _ -> false
    in
  ...
  let parenze_typ ({ctx; ast= typ} as xtyp) =
    ...
    | { ast= {ptyp_desc= Ptyp_alias _; _}
      ...
      when List.exists l ~f:(fun c ->
               ...
               | Pcstr_tuple l -> List.exists l ~f:(fun carg -> carg.pca_type |> phys_equal typ)
               | _ -> false ) ->
        true
  ...
  let parenze_pat ({ctx; ast= pat} as xpat) =
    ...
    (* CR modes: there are a lot in this match *)
